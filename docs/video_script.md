...
Coder [MCP: write] → **Reviewer** [MCP: read config + read Clérac dataset + write] → fixed
Phaser engine. Briefly show `app/agent.py` with the three `LlmAgent`s and `McpToolset`.

**Say:**
"It's a three-agent pipeline on Google's ADK. A **Researcher** reads the site's data through an
MCP filesystem server. A **Coder** turns those facts into a small, constrained level config.
And a **Reviewer** independently validates that config — the schema, the real Clérac species,
and whether the game is actually winnable — before a human sees it. For this demo the whole
pipeline is local and filesystem-only, so it runs the same way every time. Every agent
...
chamotte clay, bird-banding with the MNHN, chestnut groves left unmined for bats — and pulls
the site's biodiversity straight from that file. It hands the facts to the **Coder**, which
reads the asset manifest and writes `level_config.json` — again, through MCP. Count the tool
calls climbing."
...
- **No web on camera — by design.** Web MCP (Tavily/Fetch) is disabled for the demo because the
  corporate VPN blocks Tavily's TLS (`docs/TAVILY_VPN_INCIDENT.md`); the pipeline is 100 %
  filesystem, so nothing network-dependent can fail mid-take. If asked, frame it as the honest
  line: *we prioritized reliability over web enrichment during the demo phase due to network
  constraints* — and note the web path re-enables in one line off-VPN.
